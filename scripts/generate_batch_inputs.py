...
def validate_keys(json_data, batch_data):
    ...
    if any([k not in batch_header for k in all_valid_keys]):
        raise ValueError(f"ERROR: Required column headers missing from TSV file: {all_valid_keys}")
    batch_parameters = []
    ...
    for sample_id in data:
        ...
        for k in all_valid_keys:
            ...
            value = sample.get(k, '')
            if not value:
                raise ValueError(f"ERROR: Required value missing from TSV file: {all_valid_keys}")
